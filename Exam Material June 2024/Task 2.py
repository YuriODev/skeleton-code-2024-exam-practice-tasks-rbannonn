...
def Main():
    ...
    while Again == "y": 
        ...
        else: # if no file is provided it creates 8by8 with random amounts of @
            MyPuzzle = Puzzle(8, int(8 * 8 * 0.6)) # 38.4 as a integer is 38 60% of the grid is what it wants full 0.6 is the percentage
...
class Puzzle():
    def __init__(self, *args):
        if len(args) == 1:
            ...
            self.__Score = 0
            self.__SymbolsLeft = args[1] #when no file is inputted args = (8,int(38.4)) so agrs[1]
    ...
    def CheckforMatchWithPattern(self, Row, Column):
        for StartRow in range(Row + 2, Row - 1, -1): #loops through all possible pattern start position
            for StartColumn in range(Column - 2, Column + 1):
                try:
                    ...
                    #765
                    PatternList = []
                    PatternList.append(self.__GetCell(StartRow, StartColumn).GetSymbol())
                    PatternList.append(self.__GetCell(StartRow, StartColumn + 1).GetSymbol())
                    PatternList.append(self.__GetCell(StartRow, StartColumn + 2).GetSymbol())
                    PatternList.append(self.__GetCell(StartRow - 1, StartColumn + 2).GetSymbol())
                    PatternList.append(self.__GetCell(StartRow - 2, StartColumn + 2).GetSymbol())
                    PatternList.append(self.__GetCell(StartRow - 2, StartColumn + 1).GetSymbol())
                    PatternList.append(self.__GetCell(StartRow - 2, StartColumn).GetSymbol())
                    PatternList.append(self.__GetCell(StartRow - 1, StartColumn).GetSymbol())
                    Patternlist.append(self.__GetCell(StartRow - 1, StartColumn + 1).GetSymbol())
...
class Cell():
    def __init__(self):
        self._Symbol = "" #empty
        self.__SymbolsNotAllowed = [] # should be filled with unavalaible choices

    def GetSymbol(self): # will show the symbol that is in that position or a -
        if self.IsEmpty():
          return "-"
        else:
          return self._Symbol
    ...
    def IsEmpty(self):
        if len(self._Symbol) == 0: # if symbol == ""
            return True
        else:
            return False 
...
class BlockedCell(Cell):
    def __init__(self):
        super(BlockedCell, self).__init__() #inherited cell
        self._Symbol = "@"
